        ...
        while retries < max_retries:
            ...
            try:
                ...
                if is_429_error and settings.ENABLE_KEY_FREEZE_ON_429:
                    ...
                    await self.key_manager.handle_429_error(current_attempt_key, is_vertex=True)
                    api_key = await self.key_manager.get_next_working_vertex_key()
                else:
                    # 对于其他错误，使用正常的失败处理逻辑
                    api_key = await self.key_manager.handle_vertex_api_failure(current_attempt_key, retries)
